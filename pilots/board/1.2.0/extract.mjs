export async function search(page, query) {
  ...
  const state = await page.$$eval('body', (bodies) => {
    ...
    const records = [...body.querySelectorAll('article.job-card')].map((card) => {
      const link = card.querySelector('h2 a[href]');
      return {
        title: clean(link),
        company: clean(card.querySelector('.company')),
        location: clean(card.querySelector('.location')),
        url: link?.href || null,
        employmentType: clean(card.querySelector('.type')),
        postedAt: clean(card.querySelector('time'))
      };
    });
