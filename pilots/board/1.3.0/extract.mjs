export async function search(page, query) {
  ...
  const state = await page.$$eval('body', (bodies) => {
    ...
    const records = [...body.querySelectorAll('table.postings tbody tr')].map((row) => {
      const link = row.querySelector('td.posting-title a[href]');
      return {
        title: clean(link),
        company: clean(row.querySelector('td.posting-org')),
        location: clean(row.querySelector('td.posting-where')),
        url: link?.href || null,
        employmentType: clean(row.querySelector('td.posting-kind')),
        postedAt: clean(row.querySelector('td.posting-date'))
      };
    });
