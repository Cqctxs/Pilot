export async function search(page, query) {
  ...
  const state = await page.$$eval('body', (bodies) => {
    ...
    const records = [...body.querySelectorAll('table tbody tr.posting')].map((row) => {
      const link = row.querySelector('.posting-title a[href]');
      return {
        title: clean(link),
        company: clean(row.querySelector('.posting-org')),
        location: clean(row.querySelector('.posting-where')),
        url: link?.href || null,
        employmentType: clean(row.querySelector('.posting-kind')),
        postedAt: clean(row.querySelector('.posting-date'))
      };
    });
